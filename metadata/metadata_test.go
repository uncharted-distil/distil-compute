...
func TestMetadataFromSchema(t *testing.T) {
	...
	assert.Equal(t, meta.DataResources[0].Variables[0].Key, "bravo")
	...
	assert.Equal(t, meta.DataResources[0].Variables[1].Key, "alpha")
	...
	assert.Equal(t, meta.DataResources[0].Variables[2].Key, "whiskey")
	...
	assert.Equal(t, meta.DataResources[0].Variables[3].Key, "d3mIndex")
