...
func CreateImageClusteringPipeline(name string, description string, imageVariables []*model.Variable, useKMeans bool) (*FullySpecifiedPipeline, error) {
	...
	if useKMeans {
		steps = []Step{
			...
			NewKMeansCluteringStep(map[string]DataRef{"inputs": &StepDataRef{5, "produce"}}, []string{"produce"}),
...
func CreateMultiBandImageClusteringPipeline(name string, description string,
	...
	if useKMeans {
		steps = []Step{
			...
			NewKMeansCluteringStep(map[string]DataRef{"inputs": &StepDataRef{7, "produce"}}, []string{"produce"}),
